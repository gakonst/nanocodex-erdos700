...
The upstream conjecture wraps the mathematical assertion in an unknown
`answer(sorry)`.  A solution should prove the right-hand set infinite directly;
it must not use the open equivalence theorem.
-/
